...
function CompactResumeCard({ resume }: CompactResumeCardProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = resume.file_paths?.docx || '#'
      link.download = `${resume.company}_${resume.role}_Resume.docx`.replace(/\s+/g, '_')
              ...
              {showActions && (
                <div className="absolute right-0 top-full mt-1 w-32 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                  <div className="py-1">
                    ...
                    <button className="flex items-center w-full px-3 py-1.5 text-xs text-red-600 hover:bg-red-50">
