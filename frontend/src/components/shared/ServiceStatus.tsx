...
export function ServiceStatus({ className }: ServiceStatusProps) {
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      ...
      {health && (
        <Badge 
          variant={health.lm_studio_connected && health.database_status === 'connected' ? 'default' : 'destructive'}
          className="text-xs"
        >
          {health.lm_studio_connected && health.database_status === 'connected' ? 'Online' : 'Issues'}
        </Badge>
      )}
