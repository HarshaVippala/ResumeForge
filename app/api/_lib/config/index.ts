...
import { validateEnvOrThrow, getRequiredEnv, getOptionalEnv, isFeatureEnabled, getSupabaseUrl, getSupabaseAnonKey } from './validate-env';
...
if (typeof window === 'undefined' && !process.env.NEXT_PHASE) {
  try {
    ...
    // In development or during build, log but don't crash
    if (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test') {
      console.warn('⚠️  Continuing despite validation errors (development mode)');
    } else {
      throw error;
    }
  }
 ...
 * Prioritizes Supabase integration-provided variables, falls back to manual configuration
 */
...
export const ai = {
  ...
  model: getOptionalEnv('AI_MODEL', 'gemini-2.0-flash'),
...
export const features = {
  ...
  setupMode: isFeatureEnabled('SETUP_MODE'),
} as const;
...
export const development = {
  ...
  // SECURITY WARNING: This disables authentication in development only
  // NEVER enable this in production!
  disableAuth: getOptionalEnv('NODE_ENV', 'development') === 'development' && 
               getOptionalEnv('DISABLE_AUTH_IN_DEV', 'false') === 'true',
} as const;
